import React from 'react';
import { View, StyleSheet, TouchableOpacity } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import * as Haptics from 'expo-haptics';
import { useTheme } from '../../theme/ThemeProvider';

export type QuickAccessButtonType = 
  | 'search' 
  | 'voice' 
  | 'keyboard' 
  | 'settings' 
  | 'back' 
  | 'guide' 
  | 'home';

export interface QuickAccessButtonsProps {
  onButtonPress: (button: QuickAccessButtonType) => void;
  disabled?: boolean;
  testID?: string;
}

interface ButtonConfig {
  type: QuickAccessButtonType;
  icon: keyof typeof Ionicons.glyphMap;
  label: string;
  accessibilityHint: string;
}

const topRowButtons: ButtonConfig[] = [
  {
    type: 'search',
    icon: 'search',
    label: 'Search',
    accessibilityHint: 'Opens search interface'
  },
  {
    type: 'voice',
    icon: 'mic',
    label: 'Voice',
    accessibilityHint: 'Activates voice search'
  },
  {
    type: 'keyboard',
    icon: 'keypad',
    label: 'Keyboard',
    accessibilityHint: 'Opens on-screen keyboard'
  },
  {
    type: 'settings',
    icon: 'settings',
    label: 'Settings',
    accessibilityHint: 'Opens settings menu'
  }
];

const bottomRowButtons: ButtonConfig[] = [
  {
    type: 'back',
    icon: 'arrow-back',
    label: 'Back',
    accessibilityHint: 'Navigates back to previous screen'
  },
  {
    type: 'guide',
    icon: 'list',
    label: 'Guide',
    accessibilityHint: 'Opens program guide'
  },
  {
    type: 'home',
    icon: 'home',
    label: 'Home',
    accessibilityHint: 'Returns to home screen'
  }
];

export const QuickAccessButtons: React.FC<QuickAccessButtonsProps> = ({
  onButtonPress,
  disabled = false,
  testID
}) => {
  const { theme } = useTheme();

  const handleButtonPress = async (type: QuickAccessButtonType) => {
    if (disabled) return;

    // Provide haptic feedback
    try {
      await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    } catch (error) {
      console.warn('Haptic feedback not available:', error);
    }

    onButtonPress(type);
  };

  const renderButton = (buttonConfig: ButtonConfig) => {
    const { type, icon, label, accessibilityHint } = buttonConfig;
    
    return (
      <View key={type} style={styles.buttonContainer}>
        <TouchableOpacity
          onPress={() => handleButtonPress(type)}
          disabled={disabled}
          activeOpacity={0.7}
          accessibilityRole="button"
          accessibilityLabel={label}
          accessibilityHint={accessibilityHint}
          accessibilityState={{ disabled }}
          style={[
            styles.button,
            {
              backgroundColor: theme.colors.surface.button,
              borderRadius: theme.borderRadius.lg,
            },
            disabled && styles.disabled
          ]}
          testID={`${testID}-${type}`}
          hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
        >
          <Ionicons
            name={icon}
            size={theme.icons.size.lg}
            color={disabled ? theme.colors.text.secondary : theme.colors.text.primary}
          />
        </TouchableOpacity>
      </View>
    );
  };

  return (
    <View style={styles.container} testID={testID}>
      {/* Top row: Search, Voice, Keyboard, Settings */}
      <View style={[styles.row, { marginBottom: theme.spacing.sm }]}>
        {topRowButtons.map(renderButton)}
      </View>
      
      {/* Bottom row: Back, Guide, Home */}
      <View style={styles.row}>
        {bottomRowButtons.map(renderButton)}
      </View>
    </View>
  );
};

const styles = StyleSheet.create({
  container: {
    alignItems: 'center',
  },
  row: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    width: '100%',
  },
  buttonContainer: {
    flex: 1,
    marginHorizontal: 4,
  },
  button: {
    aspectRatio: 1,
    minHeight: 50,
    minWidth: 50,
    alignItems: 'center',
    justifyContent: 'center',
  },
  disabled: {
    opacity: 0.5,
  },
});