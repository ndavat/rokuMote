...
export default function SettingsScreen() {
  ...
  const handleMockModeToggle = async () => {
    ...
    try {
      ...
      // Get BLE service factory and switch service type
      const factory = getBLEServiceFactory();
      if (newMockMode) {
        factory.setUseMockService(true);
        actions.connection.setConnectionStatus('disconnected');
        actions.connection.setCurrentDevice(null);
      } else {
        factory.setUseMockService(false);
        // Reset connection state when switching to real BLE
        actions.connection.setConnectionStatus('disconnected');
        actions.connection.setCurrentDevice(null);
      }
        ...
        <View style={[styles.section, { borderBottomColor: theme.colors.surface.card }]}>
          ...
          <View style={styles.settingRow}>
            ...
            <Button
              ...
              variant="secondary"
              style={[styles.actionButton, { backgroundColor: theme.colors.status.disconnected }]}
            />
          </View>
        </View>
